    ...
    <PageContainer>
      <div className="flex min-h-[70vh] w-full items-center justify-center py-10">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm leading-relaxed text-foreground/80">{t("description")}</p>
        </div>

        {showDetails ? (
          <details className="group w-full rounded-lg border border-border/60 bg-background/90 text-sm text-center">
            <summary className="cursor-pointer list-none px-4 py-3 font-medium text-foreground/70 outline-none marker:hidden [&::-webkit-details-marker]:hidden hover:text-foreground/90">
            ...
            <div className="space-y-3 border-t border-border/50 px-4 pb-4 pt-3">
              <p className="text-xs leading-relaxed text-foreground/55">{t("detailsHint")}</p>
              {digest ? (
                <p className="font-mono text-xs text-foreground/70">
                  <span className="font-sans text-foreground/50">{t("digestLabel")}: </span>
                  {digest}
                </p>
              ) : null}
              {technical ? (
                <p className="break-words font-mono text-xs text-foreground/65">{technical}</p>
              ) : null}
            </div>
          </details>
        ) : null}
